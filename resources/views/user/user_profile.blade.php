                ...
                <div class="w-full md:w-3/12 md:mx-2">
                    ...
                    @if($user->id === (auth()->user()->id))
                        ...
                        <form action="{{ route('user.update.phone',['id' => $user->id]) }}" method="POST" id="updatePhoneForm">
                            ...
                            <button type="submit" class="h-10 w-20 text-white rounded-lg bg-green-400 hover:bg-lime-400" onclick="confirmPhoneNumber()">Submit</button>
                        </form>
                        ...
                        <ul
                            ...
                            <li class="flex items-center py-3">
                                ...
                                <span class="ml-auto"><span
                                        class="bg-green-500 py-1 px-2 rounded text-white text-sm">{{$user->listings->count()}}</span></span>
                            </li>
                            {{-- REPUTATIONS TITLE --}}
                            <li class="flex items-center py-3">
                                <span>Reputation</span>
                                {{-- <span class="ml-auto"><span
                                    class="bg-red-900 py-1 px-2 rounded text-white text-sm"> --}}

                                @if ($user->reputation >= 1000)
                                <span class="ml-auto"><span
                                    class="bg-red-800 py-1 px-2 rounded text-white text-sm">
                                Admin Candidate
                                @elseif($user->reputation >= 900)
                                <span class="ml-auto"><span
                                    class="bg-red-700 py-1 px-2 rounded text-white text-sm">
                                Expert
                                @elseif($user->reputation >= 800)
                                <span class="ml-auto"><span
                                    class="bg-red-600 py-1 px-2 rounded text-white text-sm">
                                Trusted Member
                                @elseif($user->reputation >= 700)
                                <span class="ml-auto"><span
                                    class="bg-red-500 py-1 px-2 rounded text-white text-sm">
                                Active Contributor
                                @elseif($user->reputation >= 600)
                                <span class="ml-auto"><span
                                    class="bg-pink-500 py-1 px-2 rounded text-white text-sm">
                                Contributor
                                @elseif($user->reputation >= 500)
                                <span class="ml-auto"><span
                                    class="bg-indigo-500 py-1 px-2 rounded text-white text-sm">
                                Enthusiast
                                @elseif($user->reputation >= 400)
                                <span class="ml-auto"><span
                                    class="bg-purple-500 py-1 px-2 rounded text-white text-sm">
                                Apprentice
                                @elseif($user->reputation >= 300)
                                <span class="ml-auto"><span
                                    class="bg-yellow-500 py-1 px-2 rounded text-white text-sm">
                                Learner
                                @elseif($user->reputation >= 200)
                                <span class="ml-auto"><span
